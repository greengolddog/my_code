...
vector<ll> stupid(int n, int m, vector<ll> a, vector<pair<ll, ll>> q) {
	vector<ll> ans;
    	for (auto[l, r] : q) {
    	    vector<ll> b, v;
    	    for (int i = l - 1; i < r; ++i)
            ...
            bool next_otr = b.back() < 0, anyway = false;
       	    while (b.size() > 1)
            {
                if (b.back() < 0) {
                   v.push_back(b.back());
                   b.pop_back();
                    ...
                    b.pop_back();
                    b.back() = b.back() + 2 * nw;
                    if (b.back() > 2LL * (ll)n * MOD) anyway = true;
                    if (anyway) b.back() = (MOD * 2 + b.back()) % MOD;
                }
             }
             ll nw = (b.back() + MOD) % MOD;
             for (ll j : v) nw = (nw + 2 * j + 2 * MOD) % MOD;
             ans.push_back(nw);
        }
	return ans;
}
...
vector<ll> solve(int n, int m, vector<ll> arr, vector<pair<ll, ll>> qq) {
	...
	for (ll i = 0; i < n; i++) {
		...
		for (auto[l, j] : q[i]) {
			...
			it--;
			ll y = *it;
			ans[j] += st.get(l, min(x, i + 1)) * inv(binpow(2, l - y)) % MOD;
			if (x <= i) {
				ans[j] += st.get(x, i + 1);
			}
			if (arr[l] < 0 && l > 0) {
...
int main() {
	...
	if (b) {
		...
		while (true) {
			...
			vector<ll> v(n);
			for (ll i = 0; i < n; i++) {
				v[i] = rnd64() % 2'000'000'001 - 1'000'000'000;
			}
			vector<pair<ll, ll>> q;
			for (ll l = 0; l < n; l++) {
				for (ll r = l + 1; r < n; r++) {
			...
			cout << endl;
			for (auto i : solve(n, m, v, q)) {
				cout << (int)i << ' ';
			}
			cout << endl;
			for (auto i : stupid(n, m, v, q)) {
				cout << (int)i << ' ';
			}
			cout << endl;
			if (solve(n, m, v, q) != stupid(n, m, v, q)) {
				break;
			}
		}
	}
