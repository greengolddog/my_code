...
const db nan2 = 0.0/0.0, eps = 1e-6;
const db pi = acos(-1);
...
int main() {
	...
	cout << 2 << '\n';
	auto[a, b] = c.tan(p);
	point p3 = a + (line(segment(p, c.s)).norm().rev().to(line(segment(p, c.s)).dist(a))).to_point();
	cout << p3 << '\n';
	cout << p.dist(p3) << ' ' << a.dist(p3) << '\n';
	cout << a << '\n' << b;
}
