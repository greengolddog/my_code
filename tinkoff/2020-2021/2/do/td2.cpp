...
ll nod_help(ll a, ll b) {
    if (a == 0) {
        return b;
    }
    return nod_help(b%a, a);
}

ll nod(ll a, ll b) {
    return nod_help(min(a, b), max(a, b));
}
...
struct segment_tree {
    ...
    ll get_sum_help(ll v, ll vl, ll vr, ll l, ll r) {
        ...
        ll mid = (vl+vr)/2;
        return nod(get_sum_help(v*2, vl, mid, l, r), get_sum_help(v*2+1, mid+1, vr, l, r));
    }
