...
struct polygon {
    ...
    bool is_convex() {
        if (len > 2) {
            db sign_fir = sign(vct(points[1], points[0])^vct(points[2], points[0]));
            for (ll i = 3; i < len; i++) {
                if (sign(vct(points[i-1], points[0])^vct(points[i], points[0])) != sign_fir) {
                    return false;
                }
            }
        }
...
int main() {
	...
	pair<polygon, point> p;
	cin >> p;
	cout << (p.first.point_in(p.second) ? "YES" : "NO");
}
