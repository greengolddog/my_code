...
int main() {
	...
	for (ll i = 0; i < t; i++) {
		circle c1, c2;
		cin >> c1 >> c2;
		pair<ll, pair<pair<point, pair<db, db>>, pair<point, point>>> ans = c1*c2;
		cout << ans.first << "\n";
		if (ans.first == 1) {
			cout << ans.second.second.first << "\n";
		}
		if (ans.first == 2) {
			cout << ans.second.first.first << "\n"
			<< setprecision(20) << abs(ans.second.first.second.first) << ' ' << setprecision(20) << abs(ans.second.first.second.second) << "\n"
			<< ans.second.second.first << "\n"
			<< ans.second.second.second << "\n";
		}
	}
}
