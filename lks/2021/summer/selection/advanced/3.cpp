...
int main() {
	...
	for (ll i = 0; i < n; i++) {
		...
		s.insert({arr[i].first*1.0, arr[i].second*1.0});
	}
	db ans = 0;
	for (ll i = 0; i < n; i++) {
		for (ll j = 0; j < n; j++) {
			for (ll k = 0; k < n; k++) {
				line l1 = line(segment(point(arr[i].first, arr[i].second), point(arr[j].first, arr[j].second)));
				line l2 = line(segment(point(arr[i].first, arr[i].second), point(arr[k].first, arr[k].second)));
				line l3 = line(segment(point(arr[j].first, arr[j].second), point(arr[j].first, arr[j].second)+l1.norm().to_point()));
				line l4 = line(segment(point(arr[k].first, arr[k].second), point(arr[k].first, arr[k].second)+l2.norm().to_point()));
				point l = l3*l4;
				l.round();
				// cout << i << ' ' << j << ' ' << k << ' ' << l << endl;
				if (s.find({rou(l.x), rou(l.y)}) != s.end()) {
					if (rou(point(arr[i].first, arr[i].second).dist(point(arr[j].first, arr[j].second))) == rou(l.dist(point(arr[k].first, arr[k].second)))) {
						// cout << i << ' ' << j << ' ' << k << ' ' << l << endl;
						if (rou(point(arr[i].first, arr[i].second).dist(point(arr[k].first, arr[k].second))) == rou(l.dist(point(arr[j].first, arr[j].second)))) {
							// cout << i << ' ' << j << ' ' << k << ' ' << l << endl;
							if (rou(!angle(point(arr[i].first, arr[i].second), point(arr[j].first, arr[j].second), l)) == 90.0) {
								// cout << i << ' ' << j << ' ' << k << ' ' << l << endl;
								if (rou(!angle(point(arr[j].first, arr[j].second), point(arr[i].first, arr[i].second), point(arr[k].first, arr[k].second))) == 90.0) {
									// cout << i << ' ' << j << ' ' << k << ' ' << l << ' ' << abs(triangle(point(arr[i].first, arr[i].second), point(arr[j].first, arr[j].second), l).square()) + 
									// abs(triangle(point(arr[i].first, arr[i].second), point(arr[k].first, arr[k].second), l).square()) << endl;
									ans = max(ans, abs(triangle(point(arr[i].first, arr[i].second), point(arr[j].first, arr[j].second), l).square()) + 
									abs(triangle(point(arr[i].first, arr[i].second), point(arr[k].first, arr[k].second), l).square()));
								}
							}
						}
					}
