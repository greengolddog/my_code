...
int main() {
	...
	circle c;
	cin >> c;
	auto ans1 = c.tan(p1), ans2 = c.tan(p2);
	if (((rou(from_rad(abs(vct(c.s, p1) % vct(p2, p1)))) <= 90 &&
	      rou(from_rad(abs(vct(c.s, p2) % vct(p1, p2)))) <= 90) ?
	      line(segment(p1, p2)).dist(c.s) :
	      min(p1.dist(c.s), p2.dist(c.s))) >= c.r) {
		cout << setprecision(precision) << p1.dist(p2);
		return 0;
	}
	db a11 = p1.dist(ans1.first) + c.r * abs(vct(ans1.first, c.s) % vct(ans2.first, c.s)) + p2.dist(ans2.first);
	db a12 = p1.dist(ans1.first) + c.r * abs(vct(ans1.first, c.s) % vct(ans2.second, c.s)) + p2.dist(ans2.second);
	db a22 = p1.dist(ans1.second) + c.r * abs(vct(ans1.second, c.s) % vct(ans2.second, c.s)) + p2.dist(ans2.second);
	db a21 = p1.dist(ans1.second) + c.r * abs(vct(ans1.second, c.s) % vct(ans2.first, c.s)) + p2.dist(ans2.first);
	cout << setprecision(precision) << min(min(a11, a12), min(a22, a21));
}
