...
bool cmp(pair<vct, point> p1, pair<vct, point> p2) {
	if (rou(p1.first ^ p2.first) == 0) {
...
struct polygon {
		...
		if (!b) {
			...
			vector<pair<vct, point>> arr;
			for (auto i : p) {
				if (i != minp) {
					arr.push_back(make_pair(vct(i, minp), i));
				}
			}
...
int main() {
	...
	for (ll i = 0; i < poly.points.size(); i++) {
		per += poly.points[j].dist(poly.points[i]);
		j = i;
	}
	cout << '\n';
	cout << setprecision(precision) << per << '\n';
	cout << setprecision(precision) << poly.area();
}
