...
int main() {
	freopen("goat8.in", "r", stdin);
	freopen("goat8.out", "w", stdout);
	...
	if (ans.first == 1) {
        ...
        if (ans.first == 2) {
		cout << point(min(ans.second.second.first.to_pair(), ans.second.second.second.to_pair())) << '\n'
		<< point(max(ans.second.second.first.to_pair(), ans.second.second.second.to_pair()));
	}
}
