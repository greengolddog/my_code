...
int main() {
	...
	cin >> k;
	now = 0;
	while (k-n-ans[now]-1+lcp[ans[now]] >= 0 && now < n-1) {
		k -= n-ans[now]-1-lcp[ans[now]];
		now++;
	}
	cout << now << ' ' << ans[now] << ' ' << lcp[ans[now]] << endl;
	for (ll i = ans[now]+lcp[ans[now]]; i < min(n-1, ans[now]+lcp[ans[now]]+k); i++) {
		cout << s[i];
	}
}
